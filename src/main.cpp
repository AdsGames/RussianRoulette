...
bool location_clicked(BITMAP* buffer,
                      int color,
                      int min_x,
                      int max_x,
                      int min_y,
...
void game() {
  // Pull back trigger
  if (mouse_b & 1 && mouse_x < 680 && mouse_x > 550 && mouse_y > 200 &&
      mouse_y < 400 && trigger_loop > 9) {
  ...
  if (wheel_loop > 9 && !wheel_in && key[KEY_E]) {
    ...
    wheel_loop = 0;
    if (sound)
      play_sample(wheel_close, 255, 125, 1000, 0);
  }
  ...
  if (wheel_loop > 9 && wheel_in && key[KEY_E]) {
    ...
    wheel_loop = 0;
    if (sound)
      play_sample(wheel_open, 255, 125, 1000, 0);
  }

  wheel_loop++;

  // Put in some bullets
  if (key[KEY_1] && toggle_loop > 9 && shell[1].state == blank) {
    shell[1].state = full;
    toggle_loop = 0;
    if (sound)
  ...
  if (key[KEY_1] && toggle_loop > 9 && shell[1].state != blank) {
    ...
    toggle_loop = 0;
    if (sound)
      play_sample(shell_drop, 255, 125, random(600, 1200), 0);
  }
  if (key[KEY_2] && toggle_loop > 9 && shell[2].state == blank) {
    shell[2].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_3] && toggle_loop > 9 && shell[3].state == blank) {
    shell[3].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_4] && toggle_loop > 9 && shell[4].state == blank) {
    shell[4].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_5] && toggle_loop > 9 && shell[5].state == blank) {
    shell[5].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_6] && toggle_loop > 9 && shell[6].state == blank) {
    shell[6].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_7] && toggle_loop > 9 && shell[7].state == blank) {
    shell[7].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  if (key[KEY_8] && toggle_loop > 9 && shell[8].state == blank) {
    shell[8].state = full;
    toggle_loop = 0;
    if (sound)
      play_sample(shell_click, 255, 125, 1000, 0);
  ...
  rotate_sprite(gun, hammer, 554, 26,
                itofix(angle_trigger - angle_trigger * 2));
  rotate_sprite(gun, trigger, 576, 185, itofix(angle_trigger));
  if (wheel_in)
    draw_sprite(gun, python, 0, 0);
  if (!wheel_in)
    draw_sprite(gun, python_open, 0, 0);

  if (!wheel_in) {
    rectfill(cylinder, 0, 0, 200, 200, makecol(255, 0, 255));
    draw_sprite(cylinder, wheel, 0, 0);
    if (shell[1].state == full)
      draw_sprite(cylinder, bullet, 76, 9);
    if (shell[8].state == full)
      draw_sprite(cylinder, bullet, 120, 29);
    if (shell[7].state == full)
      draw_sprite(cylinder, bullet, 138, 72);
    if (shell[6].state == full)
      draw_sprite(cylinder, bullet, 122, 118);
    if (shell[5].state == full)
      draw_sprite(cylinder, bullet, 78, 137);
    if (shell[4].state == full)
      draw_sprite(cylinder, bullet, 32, 120);
    if (shell[3].state == full)
      draw_sprite(cylinder, bullet, 14, 74);
    if (shell[2].state == full)
      draw_sprite(cylinder, bullet, 31, 29);

    if (shell[1].state == shot)
      draw_sprite(cylinder, bullet_shot, 76, 9);
    if (shell[8].state == shot)
      draw_sprite(cylinder, bullet_shot, 120, 29);
    if (shell[7].state == shot)
      draw_sprite(cylinder, bullet_shot, 138, 72);
    if (shell[6].state == shot)
      draw_sprite(cylinder, bullet_shot, 122, 118);
    if (shell[5].state == shot)
      draw_sprite(cylinder, bullet_shot, 78, 137);
    if (shell[4].state == shot)
      draw_sprite(cylinder, bullet_shot, 32, 120);
    if (shell[3].state == shot)
      draw_sprite(cylinder, bullet_shot, 14, 74);
    if (shell[2].state == shot)
      draw_sprite(cylinder, bullet_shot, 31, 29);
  ...
  textprintf_ex(buffer, font, 445, 517, makecol(0, 0, 0), makecol(0, -1, 0),
                "Clear");
  ...
  textprintf_ex(buffer, font, 495, 517, makecol(0, 0, 0), makecol(0, -1, 0),
                "Fill");
  ...
  textprintf_ex(buffer, font, 350, 490, makecol(0, 0, 0), makecol(0, -1, 0),
                "Keys 1-8 to insert/remove shells", wheelLocation);
